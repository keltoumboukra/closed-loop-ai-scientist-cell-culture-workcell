...
**More detail:** [`src/parser/README.md`](src/parser/README.md) (formats, metrics, layout), [`src/webapp/README.md`](src/webapp/README.md) (API routes, frontend structure), [`src/design/README.md`](src/design/README.md) (reagent LHS, `transfer_array`, validation).
...
- **Reagent LHS design for Monomer:** run `uv sync` first (Python deps; `matplotlib` is included for `--plots`). Then: `uv run python scripts/generate_reagent_design.py`. **Which iteration** is chosen by the optional first argument: omit it to use `data/iterations/iter_001`, pass `iter_NNN` to use `data/iterations/iter_NNN`, or pass a path to any `iter_*` folder. Under that folder, `input/` gets `well_to_design_mapping.json`, `transfer_array.json` (validated before save), `<folder>_design_summary.md` (includes workcell setup with per-well aspirated volumes), and with **`--plots`** the `<folder>_lhs_*.png` figures. **`--seed N`** (default `42`) fixes the Latin hypercube and well shuffle for reproducibility; not every seed is feasible under the 8.5 mL source well cap -- the script will error clearly if the chosen seed is not. Source plate is a single **24-well plate** (`source_24w`); experiment plate is **96-well** and starts empty. Module layout and imports: [`src/design/README.md`](src/design/README.md).
